...
function renderPlaneacionTable(planeacionArray) {
  const tableBody = document.getElementById("planeacionTableBody");
  tableBody.innerHTML = ""; // Limpiar el contenido anterior

  planeaciones.forEach((planeacion, index) => {
    const row = document.createElement("tr");

    row.innerHTML = `
      <td class="px-4 py-3">
        <img src="${planeacion.portada}" alt="Logo" class="w-10 h-10 rounded-full object-cover" />
      </td>
      <td class="px-4 py-3">${planeacion.name}</td>
      <td class="px-4 py-3">${planeacion.fecha}</td>
      <td class="px-4 py-3">
...
document.addEventListener("DOMContentLoaded", () => {
  renderPlaneacionTable(planeaciones);
  addTableListeners();
});
